import react from "@vitejs/plugin-react";
import { configDefaults, defineConfig } from "vitest/config";
import { VitePWA } from "vite-plugin-pwa";

const GITHUB_PAGES_REPOSITORY = "Pipo-Chess-3d";
const githubPagesBase = `/${GITHUB_PAGES_REPOSITORY}/`;
const deployBase = process.env.DEPLOY_TARGET === "github-pages" ? githubPagesBase : "/";

export default defineConfig({
  base: deployBase,
  plugins: [
    react(),
    VitePWA({
      registerType: "prompt",
      injectRegister: false,
      includeAssets: ["favicon.svg", "icons.svg", "pipo-icon.svg", "pipo-touch-icon.png"],
      manifest: {
        name: "Pipo Chess 3D",
        short_name: "Pipo Chess",
        description: "Single-player 3D chess against an offline Stockfish engine.",
        theme_color: "#081320",
        background_color: "#081320",
        display: "standalone",
        orientation: "portrait-primary",
        id: deployBase,
        scope: deployBase,
        start_url: deployBase,
        lang: "pt-BR",
        icons: [
          {
            src: "pipo-touch-icon.png",
            sizes: "192x192",
            type: "image/png",
            purpose: "any maskable",
          },
          {
            src: "pipo-app-icon-512.png",
            sizes: "512x512",
            type: "image/png",
            purpose: "any maskable",
          },
        ],
      },
      workbox: {
        globPatterns: ["**/*.{js,css,html,svg,png,woff2,wasm,json}"],
        cleanupOutdatedCaches: true,
        clientsClaim: true,
        maximumFileSizeToCacheInBytes: 10 * 1024 * 1024,
      },
      devOptions: {
        enabled: false,
      },
    }),
  ],
  build: {
    rollupOptions: {
      output: {
        manualChunks(id) {
          const normalizedId = id.replaceAll("\\", "/");

          if (!normalizedId.includes("node_modules")) {
            return undefined;
          }

          if (normalizedId.includes("/three/examples/jsm/")) {
            return "three-extras";
          }

          if (normalizedId.includes("/three/")) {
            return "three-core";
          }

          if (normalizedId.includes("/framer-motion/")) {
            return "motion";
          }

          if (
            normalizedId.includes("/chess.js/") ||
            normalizedId.includes("/dexie/") ||
            normalizedId.includes("/zustand/")
          ) {
            return "app-vendor";
          }

          return undefined;
        },
      },
    },
  },
  test: {
    globals: true,
    environment: "jsdom",
    setupFiles: ["./src/test/setup.ts"],
    css: true,
    include: ["src/**/*.{test,spec}.{ts,tsx}"],
    exclude: [...configDefaults.exclude, "e2e/**", "playwright.config.ts"],
    coverage: {
      reporter: ["text", "html"],
      reportsDirectory: "./coverage",
    },
  },
});
